...
ttab <- function(
  ...
  if (!is.null(caption) && (!is.character(caption) || length(caption) > 1))
    stop("'caption' must be a character scalar")
  if (!is.null(label) && (!is.character(label) || length(label) > 1))
    stop("'label' must be a character scalar")
  if (!is.character(align) || !(length(align) %in% c(1, ncol(x))))
    stop("'align' must have the one element per column of x")
  if (
    !(is.character(widths) || is.numeric(widths)) ||
      !(length(widths) %in% c(1, ncol(x)))
  )
    stop("'widths' must have the one element per column of x")
  if (
    !is.null(placement) && (!is.character(placement) || length(placement) > 1)
  )
    stop("'placement' must be a character scalar")
  if (!is.null(footnotes) && !is.character(footnotes))
...
add_footnote <- function(x, footnote) {
  ...
  if (!is.null(footnote) && !is.character(footnote))
    stop("'footnote' must be a character vector")
...
add_header <- function(x, header) {
  ...
  if (!is.character(header) && !is.list(header))
    stop("'header' must be a list or character vector")
...
add_indent <- function(x, rows, indent = 1, columns = 1) {
  ...
  if (!is.numeric(indent) || length(indent) != 1)
    stop("'indent' must be a numeric scalar")
...
span_rows <- function(x, range, cols, align = "top") {
  ...
  if (!is.character(align) || length(align) != 1)
    stop("'align' must be a character scalar")
  ...
  for (c in cols) {
    ...
    for (rr in replace) {
      if (!(out$body[[rr]][[c]] %in% c("", NA)))
        warning(
          paste0(
            "`span_rows()` will overwrite non-empty cells in row ",
            rr,
            ", column ",
            c
          )
...
span_cols <- function(x, range, rows, align = "left", stroke = FALSE) {
  ...
  if (!is.character(align) || length(align) != 1)
    stop("'align' must be a character scalar")
  if (!is.logical(stroke) || length(stroke) != 1)
    stop("'stroke' must be a logical scalar")
...
make_colspan_row <- function(row, r, c, replace, align, stroke) {
  ...
  for (cc in replace) {
    if (!(row[cc] %in% c("", NA)))
      warning(
        paste0(
          "`span_cols()` will overwrite non-empty cells in row ",
          r,
          ", column ",
          cc
        )
...
colspan <- function(contents, n, align = "center", stroke = TRUE) {
  if (!is.character(contents) || length(contents) != 1)
    stop("'contents' must be a character scalar")
...
add_hline <- function(x, before) {
  if (!inherits(x, "typst_table")) stop("'x' must be a `typst_table` object")
  if (!is.numeric(before) || !all(before <= length(x$body)))
    stop("'before' must be a numeric vector indexing rows of 'x'")
...
add_vline <- function(x, before, start = 0, end = "none") {
  ...
  if (!is.numeric(before))
    stop("'before' must be a numeric vector indexing columns of 'x'")
  if (!is.numeric(start) || length(start) != 1)
    stop("'start' must be a numeric scalar")
  if (length(end) != 1 || (!is.numeric(end) && end != "none)"))
    stop("'end' must be a numeric scalar, or `\"none\"`")
...
pack_rows <- function(
  ...
  if (
    !is.numeric(start_row) ||
      length(start_row) != 1 ||
      start_row > length(x$body)
  )
    stop("'start_row' must be a numeric scalar indexing rows of 'x'")
  if (!is.numeric(end_row) || length(end_row) != 1 || end_row < start_row)
    stop("'end_row' must be a numeric scalar indexing rows of 'x'")
  if (!is.null(label) && (!is.character(label) || length(label) != 1))
    stop("'label' must be a character scalar")
  if (!is.logical(italic) || length(italic) != 1)
    stop("'italic' must be a logical scalar")
  if (!is.logical(bold) || length(bold) != 1)
    stop("'bold' must be a logical scalar")
  if (!is.logical(indent) || length(indent) != 1)
    stop("'indent' must be a logical scalar")
...
cell_spec <- function(x, row, col, italic = NULL, bold = NULL, align = NULL) {
  if (!inherits(x, "typst_table")) stop("'x' must be a `typst_table` object")
  if (!is.numeric(row) || length(row) != 1 || row > length(x$body) || row <= 0)
    stop("'row' must be a numeric scalar indexing rows of 'x'")
  if (
    !is.numeric(col) ||
      length(col) != 1 ||
      col > length(x$body[[row]]) ||
      col <= 0
  )
    stop("'col' must be a numeric scalar indexing columns of 'x'")
  if (!is.null(italic) && (!is.logical(italic) || length(italic) != 1))
    stop("'italic' must be a logical scalar")
  if (!is.null(bold) && (!is.logical(bold) || length(bold) != 1))
    stop("'bold' must be a logical scalar")
  if (!is.null(align) && (!is.logical(character) || length(align) != 1))
    stop("'align' must be a character string")

  out <- x
  out$body[[row]][[col]] <- apply_cell_spec(
    out$body[[row]][[col]],
    italic,
    bold,
    align
...
col_spec <- function(
  ...
  if (!inherits(x, "typst_table")) stop("'x' must be a `typst_table` object")
  if (!is.numeric(cols))
    stop("'cols' must be a numeric vector indexing columns of 'x'")
  if (!is.logical(italic) || length(italic) != 1)
    stop("'italic' must be a logical scalar")
  if (!is.logical(bold) || length(bold) != 1)
    stop("'bold' must be a logical scalar")
  if (!is.logical(header) || length(header) != 1)
    stop("'header' must be a logical scalar")
...
print_contents <- function(x) {
  out <- glue::glue(x)
  if (!is.null(attr(x, "indent")))
    out <- glue::glue(
      "{indent}{out}",
      indent = paste(rep("\u2003", attr(x, "indent")), collapse = ""),
      .null = NULL
    )
  if (isTRUE(attr(x, "italic")) || isTRUE(attr(x, "bold")))
    out <- glue::glue(
      "#text(",
      "{if (isTRUE(italic)) 'style: \"italic\",'}",
      "{if (isTRUE(bold)) 'weight: \"bold\",'}",
      ")[{out}]",
      italic = attr(x, "italic"),
      bold = attr(x, "bold"),
      .null = NULL
    )
  if (
    !is.null(attr(x, "colspan")) ||
      !is.null(attr(x, "rowspan")) ||
      !is.null(attr(x, "align")) ||
      !is.null(attr(x, "stroke"))
  ) {
    out <- glue::glue(
      "#table.cell(",
