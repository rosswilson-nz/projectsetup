...
#' @export
tar_render_manuscript <- function(
  name,
  path,
  output_file,
  appendix = FALSE,
  include = character(),
  file_reference_docx = NULL,
  file_csl = NULL,
  file_bib = NULL,
  pandoc_args = NULL,
  render_args = NULL,
  ...
  if (!fs::file_exists(path))
    stop_file_not_found("RMarkdown or Quarto source file", path)
  ...
  switch(
    ...
    rmarkdown = {
      rlang::warn(
        ...
        .frequency = "once",
        .frequency_id = "render_rmarkdown"
      )
      if (!rlang::is_scalar_character(output_file))
        stop_not_string("output_file")
        ...
        list(
          output_format = rmarkdown::word_document(
            pandoc_args = c(
              ...
              if (!is.null(!!file_bib))
                list("--citeproc", "--bibliography", fs::path_wd(!!file_bib))
            )
          ),
      ...
      tarchetypes::tar_render_raw(
        rlang::as_name(rlang::ensym(name)),
        path,
        packages,
        library,
        error,
    ...
    quarto = {
      ...
      output <- fs::path(
        ...
        vapply(
          details$formats,
          function(f) f$pandoc$`output-file`,
          character(1)
        )
      )
...
tar_quarto_run <- function(args, deps, sources, output, input, appendix) {
  ...
  if (appendix) {
    ...
    fs::file_delete(fs::path(
      fs::path_dir(sources[[1]]),
      fs::path_file(output[formats == "pdf"])
    ))
    output[formats == "pdf"] <- fs::path_ext_set(
      fs::path(
        fs::path_dir(sources[[1]]),
        fs::path_file(output[formats == "pdf"])
      ),
    ...
    fs::file_move(
      fs::path(fs::path_dir(sources[[1]]), fs::path_file(output)),
      output
    )
  }
