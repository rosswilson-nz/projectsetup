stop_path_exists <- function(path) {
  ...
  cli_abort(x, "projectsetup_error_path_exists", path = path)
}

stop_not_string <- function(arg, null = FALSE) {
  x <- paste0("{.arg {arg}} must be a string scalar", if (null) " or NULL.")
  cli_abort(x, "projectsetup_error_not_string", arg = arg, null = null)
}
...
stop_unknown_workflow <- function(workflow) {
  ...
  cli_abort(x, "projectsetup_error_unknown_workflow", workflow = workflow)
}

stop_not_boolean <- function(arg) {
  x <- "{.arg {arg}} must be a logical scalar."
  cli_abort(x, "projectsetup_error_not_boolean", arg = arg)
}

stop_not_number <- function(arg) {
  x <- "{.arg {arg}} must be a numeric scalar."
  cli_abort(x, "projectsetup_error_not_number", arg = arg)
}
...
stop_invalid_ci <- function(conf_level) {
  ...
  cli_abort(x, "projectsetup_error_invalid_ci", conf_level = conf_level)
}
