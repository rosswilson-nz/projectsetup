...
tar_test("tar_render_manuscript works for qmd input", {
  ...
  tar_script(
    {
      ...
      list(
        tar_render_manuscript(appendix, appendix_qmd_file, appendix = TRUE),
  ...
  expect_setequal(
    fs::path_abs(tar_read(appendix)[1:2]),
  ...
  expect_setequal(
    fs::path_abs(tar_read(manuscript)[1:2]),
  ...
  expect_false(any(fs::file_exists(c(
    "reports/manuscript.pdf",
    "reports/manuscript.docx",
    "reports/appendix.pdf",
    "reports/appendix.docx"
  ))))
